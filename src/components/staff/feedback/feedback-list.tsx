...
export function FeedbackList({
  ...
  function buildHref(overrides: { reviewed?: string; page?: number }) {
    ...
    if (overrides.reviewed !== undefined) sp.set("reviewed", overrides.reviewed);
    else if (reviewedFilter !== undefined) sp.set("reviewed", String(reviewedFilter));
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        <Link href={buildHref({ reviewed: undefined, page: 1 })} className={tabClass(reviewedFilter === undefined)}>
          All
        </Link>
        <Link href={buildHref({ reviewed: "false", page: 1 })} className={tabClass(reviewedFilter === false)}>
          Unreviewed
        </Link>
        <Link href={buildHref({ reviewed: "true", page: 1 })} className={tabClass(reviewedFilter === true)}>
          Reviewed
        </Link>
      </div>

...
export default async function PlaylistsPickerPage({
  searchParams,
}: {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-foreground text-2xl">Playlists</h1>
        <p className="text-text-secondary mt-1 text-sm">
          Search for a chapter to manage its curated playlists.
        </p>
      </div>

      <ChapterSearchInput placeholder="Search by chapter name…" />

      <ChapterPickerList chapters={chapters} basePath="/manager/playlists" query={query} />
    </div>
  );
}

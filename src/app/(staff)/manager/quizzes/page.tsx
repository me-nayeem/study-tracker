...
export default async function QuizzesPickerPage({
  searchParams,
}: {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-foreground text-2xl">Quizzes</h1>
        <p className="text-text-secondary mt-1 text-sm">
          Search for a chapter to manage its validation quiz.
        </p>
      </div>

      <ChapterSearchInput placeholder="Search by chapter name…" />

      <ChapterPickerList chapters={chapters} basePath="/manager/quizzes" query={query} />
    </div>
  );
}
